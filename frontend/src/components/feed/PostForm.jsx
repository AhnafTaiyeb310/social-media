    ...
    <div className="flex flex-col bg-white border border-gray-200 rounded-xl shadow-sm dark:bg-neutral-900 dark:border-neutral-800 transition-all overflow-hidden">
        ...
        <div className="flex items-center gap-x-3">
          ...
          <div className="grow">
            ...
            <div className="hs-dropdown relative inline-flex">
              <button type="button" className="hs-dropdown-toggle py-1 px-2 inline-flex items-center gap-x-1 text-xs font-medium rounded-md border border-gray-200 bg-white text-gray-800 shadow-sm hover:bg-gray-50 dark:bg-neutral-800 dark:border-neutral-700 dark:text-neutral-300 dark:hover:bg-neutral-700">
          ...
          <div className="flex flex-wrap gap-2">
            {form.tags.map(tag => (
              <span key={tag} className="inline-flex items-center gap-x-1 py-1 px-2.5 rounded-full text-xs font-medium bg-blue-100 text-blue-800 dark:bg-blue-900/30 dark:text-blue-400">
                #{tag}
                <button type="button" onClick={() => removeTag(tag)} className="hover:text-blue-600 dark:hover:text-white"><LuX className="size-3" /></button>
              </span>
            ))}
          </div>
        )}
        ...
        {form.imagePreviews.length > 0 && (
          <div className="grid gap-2 grid-cols-3">
            {form.imagePreviews.map((preview, index) => (
              <div key={index} className="relative aspect-square rounded-xl overflow-hidden group border border-gray-100 dark:border-neutral-800">
                <img src={preview} alt="Preview" className="w-full h-full object-cover" />
                <button type="button" onClick={() => removeImage(index)} className="absolute top-1 right-1 p-1 bg-black/50 text-white rounded-full opacity-0 group-hover:opacity-100 transition-opacity"><LuX className="size-3" /></button>
              </div>
            ))}
          </div>
        )}
      ...
      {showMoreOptions && (
        <div className="px-4 py-4 bg-gray-50 dark:bg-neutral-800/50 border-y border-gray-100 dark:border-neutral-800 space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-1.5">
              ...
              <div className="hs-dropdown relative inline-flex w-full">
                <button type="button" className="hs-dropdown-toggle w-full py-2 px-3 inline-flex justify-between items-center gap-x-2 text-sm font-medium rounded-lg border border-gray-200 bg-white text-gray-800 shadow-sm dark:bg-neutral-900 dark:border-neutral-700 dark:text-white capitalize">
                ...
                <div className="hs-dropdown-menu transition-[opacity,margin] duration hs-dropdown-open:opacity-100 opacity-0 hidden z-10 mt-2 min-w-full bg-white shadow-md rounded-lg p-1 dark:bg-neutral-800">
                  {['published', 'draft', 'scheduled', 'archived'].map((s) => (
                    <button key={s} type="button" onClick={() => setForm(p => ({...p, status: s}))} className="w-full text-start py-2 px-3 rounded-lg text-sm hover:bg-gray-100 dark:hover:bg-neutral-700 capitalize">{s}</button>
                  ))}
                </div>
              </div>
            </div>
            <div className="space-y-1.5">
              ...
              <div className="hs-dropdown relative inline-flex w-full">
                <button type="button" className="hs-dropdown-toggle w-full py-2 px-3 inline-flex justify-between items-center gap-x-2 text-sm font-medium rounded-lg border border-gray-200 bg-white text-gray-800 shadow-sm dark:bg-neutral-900 dark:border-neutral-700 dark:text-white">
                  <span className="truncate">{form.category ? (typeof form.category === 'object' ? form.category.title : form.category) : 'Uncategorized'}</span>
                ...
                <div className="hs-dropdown-menu transition-[opacity,margin] duration hs-dropdown-open:opacity-100 opacity-0 hidden z-10 mt-2 min-w-full max-h-48 overflow-y-auto bg-white shadow-md rounded-lg p-1 dark:bg-neutral-800">
                  <button type="button" onClick={() => setForm(p => ({...p, category: null}))} className="w-full text-start py-2 px-3 rounded-lg text-sm hover:bg-gray-100 dark:hover:bg-neutral-700">Uncategorized</button>
                  {categories.map((cat) => (
                    <button key={cat.id} type="button" onClick={() => setForm(p => ({...p, category: cat}))} className="w-full text-start py-2 px-3 rounded-lg text-sm hover:bg-gray-100 dark:hover:bg-neutral-700">{cat.title}</button>
                  ))}
                </div>
              </div>
            </div>
          ...
          <div className="space-y-1.5">
            ...
            <input type="text" value={tagInput} onChange={(e) => setTagInput(e.target.value)} onKeyDown={handleAddTag} placeholder="Add tags (press Enter)..." className="w-full px-4 py-2 bg-white dark:bg-neutral-900 border-gray-200 dark:border-neutral-700 rounded-lg text-sm focus:border-blue-500" />
          </div>
          {form.status === 'scheduled' && (
            <div className="p-3 bg-blue-50 dark:bg-blue-900/20 border border-blue-100 dark:border-blue-800/30 rounded-xl flex items-center gap-x-3">
              ...
              <input type="datetime-local" name="scheduled_for" value={form.scheduled_for} onChange={handleChange} className="bg-transparent border-none p-0 text-sm font-semibold focus:ring-0" />
            </div>
          )}
        </div>
      )}
      ...
      <div className="p-3 bg-gray-50 dark:bg-neutral-800/50 flex items-center justify-between border-t border-gray-200 dark:border-neutral-700">
        <div className="flex items-center gap-x-1">
          ...
          <button type="button" onClick={() => fileInputRef.current?.click()} className="p-2.5 text-gray-500 hover:bg-gray-200 dark:text-neutral-400 dark:hover:bg-neutral-700 rounded-xl"><LuImage className="size-5" /></button>
          <button type="button" className="p-2.5 text-gray-500 hover:bg-gray-200 dark:text-neutral-400 dark:hover:bg-neutral-700 rounded-xl"><LuType className="size-5" /></button>
          <button type="button" onClick={() => setShowMoreOptions(!showMoreOptions)} className={`p-2.5 rounded-xl transition-all ${showMoreOptions ? 'bg-blue-600 text-white shadow-lg shadow-blue-500/30' : 'text-gray-500 hover:bg-gray-200 dark:text-neutral-400 dark:hover:bg-neutral-700'}`}><LuSettings className="size-5" /></button>
        </div>
        <div className="flex gap-x-2">
          <button type="button" onClick={internalOnSubmit} disabled={isPending} className="py-2 px-6 text-sm font-bold rounded-xl bg-blue-600 text-white hover:bg-blue-700 disabled:opacity-50 transition-all shadow-lg shadow-blue-500/20">
